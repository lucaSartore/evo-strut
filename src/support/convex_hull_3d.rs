...
impl ConvexHull3D {
    pub fn new(points: Vec<Point>) -> Self {
        ...
        let vertexes: Vec<convexhull3d::Vertex> = points
            .iter()
            .copied()
            .map(Into::into)
            .collect();
        let hull = convexhull3d::ConvexHull3D::build(&vertexes)
            .expect("convex hull creation failed");
        let center = points.iter().copied().reduce(|p1, p2| p1 + p2)
            .expect("reduce can't fail if point's length is >= 3")
            .to_scaled(1.0 / points.len() as f32);
        Self { hull: hull.into(), points, center}
    }

    fn iter_faces(&self) -> Option<impl Iterator<Item = [Point; 3]>> {
        self.hull.as_ref()?.faces().iter().map(|face| {
    ...
    pub fn volume(&self) -> f32 {
        let mut volume = 0.0;
        let Some(faces) = self.iter_faces() else { return 0.0 };
        for [v1, v2, v3] in faces {
