...
use crate::{
    ...
    stages::contact_point_optimization::ContactPointShape,
    support::{graph_circle::find_circle, remove_random::RemoveRandom},
};
...
impl<'a> ContactPointMutator<'a> {
    fn add_support_mutation(&self, gene: &mut ContactPointsGene) {
        ...
        gene.add_contact_point(
            *to_add,
            ContactPointShape::random(&self.rand, self.settings),
        );
    }
    ...
    fn move_support_mutation(&self, gene: &mut ContactPointsGene) {
        ...
        let radius = self
            ...
            .move_support_mutation_intensity;
