...
impl Graph {
    ...
    pub fn mark_distances(&mut self, source: SupportNodeId) -> Vec<SupportNodeId> {
        ...
        while let Some(e) = queue.pop() {
            ...
            for neighbor in neighbors {
                ...
                queue.push(QueuedElement::new_from_value(neighbor.id, distance + neighbor.distance));
            }
        }
        visited
    }
    ...
    pub fn get_weighted_supports(&self, id: SupportNodeId) -> SmallVec<[(SupportNodeId, f32); 4]>{
        ...
        for supporter in self.get_supports(this) {
            let weighted_supports: SmallVec<[_;4]> = self.get_supported(supporter)
                .map(|x| (x.0, 1.0 / x.1))
                .collect();
            ...
            let weight_total: f32 = weighted_supports
                .iter()
                .map(|x| x.1)
                .sum();
    ...
    pub fn add_node(&mut self, id: SupportNodeId, position: Point, neighbors: &[SupportNodeId], supported: bool) {
        ...
        for n_id in neighbors {
            ...
            new_node.neighbors.push(Neighbor{id: *n_id, distance});
        }
        let old_node = self.nodes.insert(id, new_node);
        assert!(old_node.is_none(), "can't add a node that is already present");
    }
}
