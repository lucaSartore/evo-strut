    ...
    TB: PipelineBehaviourTrait,
{
    pub fn execute(
        ...
        let mut critical: MeshVector<FaceId, bool> =
            vec![false; graph.count_triangles()].into_iter().collect();
        critical_ids.iter().for_each(|i| critical[*i] = true);

        let grouped_areas =
            TB::TCriticalityGrouping::group_criticality(graph, settings, critical_ids);
...
impl DistanceBasedCriticalityGrouper {
    fn expand_critical(
        ...
        while let Some(node_id) = to_visit.pop() {
            ...
            let is_close_enough = border_points.iter().any(|p| {
    ...
    /// of the border
    fn find_critical_border(graph: &SurfaceGraph, critical: &[FaceId]) -> Vec<FaceId> {
        let critical_set: HashSet<_> = critical.iter().copied().collect();
    ...
    fn group_by_connected(graph: &SurfaceGraph, critical: &Vec<FaceId>) -> Vec<Vec<FaceId>> {
...
impl CriticalityGrouper for DistanceBasedCriticalityGrouper {
    fn group_criticality(
        ...
        let expanded = Self::expand_critical(
            graph,
            settings.criticality_settings.criticality_expansion_rate,
            critical,
        );
