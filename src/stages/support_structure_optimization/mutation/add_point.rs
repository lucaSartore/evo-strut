...
pub fn mutate(mutator: &SupportStructureMutator, gene: &mut SupportStructureOptimizationGene) {
    ...
    let strategy_picker = rand.next_distribution(&RandomDistribution::InRange {
        low: 0.0,
        high: 1.0,
    });
    ...
    let position = if strategy_picker < 0.33 {
        ...
        let f = rand.next_distribution(&RandomDistribution::InRange {
            low: 0.4,
            high: 0.9,
        });
    ...
    gene.supports.push(SupportPoint {
        position,
        num_contacts,
    });
}
